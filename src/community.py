...
def load_community_signals(path: Path = Path("src/data/community_signals.json")) -> Tuple[Dict[str, float], Dict[str, float], Dict[str, float]]:
...
def load_ai_moat(path: Path = Path("data/ai_moat.json")) -> Dict[str, float]:
    data = _load_json(path)
    if not data:
        return {}
